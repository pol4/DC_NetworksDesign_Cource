...
- **Loopback-интерфейсы (`/32`):** Блок `192.168.0.0/24`.
...
## 4. Порядок развертывания и проверки
...
3.  **Проверка связности:**
    - С любого Leaf-коммутатора выполнить `ping` до IP-адресов всех напрямую подключенных Spine-коммутаторов.
    - С любого Spine-коммутатора выполнить `ping` до IP-адресов всех напрямую подключенных Leaf-коммутаторов.
